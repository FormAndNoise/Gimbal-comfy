...
log = logging.getLogger("GimbalGPS_Anchor")
...
GIMBAL_DIR          = _resolve_output_dir()
WAYFINDER_DIR       = GIMBAL_DIR
MAX_FILENAME_LENGTH = _get_int_env("GIMBAL_MAX_FILENAME_LENGTH", _get_int_env("WAYFINDER_MAX_FILENAME_LENGTH", 64))
STAT_PRECISION      = _get_int_env("GIMBAL_STAT_PRECISION", _get_int_env("WAYFINDER_STAT_PRECISION", 6))


class GimbalGPS_Anchor:
    """
    ...
    def _extract_samples(d: Dict[str, Any], name: str) -> torch.Tensor:
        s = d.get("samples")
        if s is None:
            raise ValueError(f"GimbalGPS_Anchor: '{name}' missing 'samples' key.")
        if s.ndim != 4:
            raise ValueError(f"GimbalGPS_Anchor: '{name}.samples' must be 4-D [B, C, H, W], got {list(s.shape)}.")
        return s
    ...
    def _resolve_grid_coordinate(meta: Dict[str, Any], select_index: int) -> Optional[Dict[str, Any]]:
        grid_map = meta.get("gimbal_grid_map") or meta.get("wayfinder_grid_map")
    ...
    def _compute_absolute_position(
        ...
        if not isinstance(prior_raw, dict):
            log.warning(
                "GimbalGPS_Anchor: accumulated_position is not a dict "
    ...
    def anchor(
        ...
        gimbal_meta: Optional[Dict[str, Any]] = None,
        ...
        # Metadata resolution
        meta         = gimbal_meta if gimbal_meta is not None else (wayfinder_meta or {})
        ...
        if save_waypoint:
            try:
                GIMBAL_DIR.mkdir(parents=True, exist_ok=True)
                target   = GIMBAL_DIR / f"{safe_name}.json"
                ...
                save_path = out_path
                if enable_perf_logging:
                    log.info(f"[GimbalGPS_Anchor] saved: {out_path}")
...
WayfinderGPS_Anchor = GimbalGPS_Anchor
