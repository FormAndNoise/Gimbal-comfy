...
class WayfinderCompass_Pro:
    ...
    def INPUT_TYPES(cls) -> Dict[str, Any]:
        return {
            "required": {
                ...
                "mode": (["Standard", "Normalized", "Orthogonal_Projection", "Blend_Overlay", "Blend_Multiply", "Stochastic_Sample"], {"default": "Standard"}),
    ...
    def _extract_samples(d: Dict[str, Any], name: str) -> torch.Tensor:
        s = d.get("samples")
        if s is None:
            raise ValueError(f"WayfinderCompass_Pro: '{name}' missing 'samples' key.")
        if s.ndim != 4:
            raise ValueError(f"WayfinderCompass_Pro: '{name}.samples' must be 4-D [B, C, H, W].")
        return s
