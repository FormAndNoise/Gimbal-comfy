...
def tmp_wayfinder_dir(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    """
    import gimbal_gps_anchor as module
    d = tmp_path / "gimbal"
    monkeypatch.setattr(module, "GIMBAL_DIR", d)
...
class TestComputeAbsolutePosition:
    ...
    def test_malformed_prior_not_dict(self):
        """
        Non-dict accumulated_position is safely handled and defaults to origin.
        """
        meta = {"accumulated_position": "broken"}
        pos = WayfinderGPS_Anchor._compute_absolute_position(meta, None, 1.0, 1.0)
        assert pos["x"] == 0.0
        assert pos["y"] == 0.0
...
class TestAnchorCore:
    ...
    def test_report_is_string_with_header(self, node):
        ...
        assert "GimbalGPS_Anchor" in report
