...
# Compute output directory relative to repository root
REPO_ROOT = Path(__file__).parent.parent.parent  # Navigate from test_suite/scripts up to repo root
OUTPUT_DIR = REPO_ROOT / "test_results" / "fresh_run6"
...
def save_output_image(outputs, save_node_id, target_filename):
    ...
    for idx, img_meta in enumerate(out_info):
        ...
        dest_path = OUTPUT_DIR / cur_filename
...
def run_suite_1_solarpunk_slerp():
    ...
    wf = {
        ...
        "2": {"class_type": "CLIPTextEncode", "inputs": {"text": "towering cyberpunk megacity skyscraper spire, complex neon geometry, glowing holographic signage, rain-slicked futuristic architecture[...]
        
        # Prompt B: Ancient Redwood Biome Forest
        "3": {"class_type": "CLIPTextEncode", "inputs": {"text": "dense ancient giant redwood forest, towering moss-covered sequoia trees, lush ferns, dappled morning sunlight filtering through canopy[...]
        ...
        "5": {"class_type": "CLIPTextEncode", "inputs": {"text": "solarpunk architectural fusion of towering organic skyscraper spire seamlessly integrated into ancient redwood biome, glowing warm win[...]
        
        "6": {"class_type": "EmptyLatentImage", "inputs": {"width": 1024, "height": 1024, "batch_size": 1}},
        
        # Baseline A: Cyberpunk Spire (Seed 5544)
        "10": {"class_type": "KSampler", "inputs": {"seed": 5544, "control_after_generate": "fixed", "steps": 25, "cfg": 6.5, "sampler_name": "dpmpp_2m", "scheduler": "karras", "denoise": 1.0, "model"[...]
        ...
        "13": {"class_type": "KSampler", "inputs": {"seed": 6655, "control_after_generate": "fixed", "steps": 25, "cfg": 6.5, "sampler_name": "dpmpp_2m", "scheduler": "karras", "denoise": 1.0, "model"[...]
        ...
        "20": {"class_type": "KSampler", "inputs": {"seed": 5544, "control_after_generate": "fixed", "steps": 24, "cfg": 1.0, "sampler_name": "dpmpp_2m", "scheduler": "karras", "denoise": 1.0, "model"[...]
        "21": {"class_type": "KSampler", "inputs": {"seed": 6655, "control_after_generate": "fixed", "steps": 24, "cfg": 1.0, "sampler_name": "dpmpp_2m", "scheduler": "karras", "denoise": 1.0, "model"[...]

        # Slerp Noise 35%
        "30": {"class_type": "GimbalCompass_Pro", "inputs": {"strength": 0.35, "mode": "Slerp", "clamp_output": True, "clamp_min": -8.0, "clamp_max": 8.0, "allow_batch_expand": False, "ortho_per_chann[...]
        "31": {"class_type": "KSampler", "inputs": {"seed": 5544, "control_after_generate": "fixed", "steps": 24, "cfg": 5.5, "sampler_name": "dpmpp_2m", "scheduler": "karras", "denoise": 0.90, "model[...]
        ...
        "34": {"class_type": "GimbalCompass_Pro", "inputs": {"strength": 0.50, "mode": "Slerp", "clamp_output": True, "clamp_min": -8.0, "clamp_max": 8.0, "allow_batch_expand": False, "ortho_per_chann[...]
        "35": {"class_type": "KSampler", "inputs": {"seed": 5544, "control_after_generate": "fixed", "steps": 24, "cfg": 5.5, "sampler_name": "dpmpp_2m", "scheduler": "karras", "denoise": 0.90, "model[...]
        ...
        # Slerp Noise 65%
        "38": {"class_type": "GimbalCompass_Pro", "inputs": {"strength": 0.65, "mode": "Slerp", "clamp_output": True, "clamp_min": -8.0, "clamp_max": 8.0, "allow_batch_expand": False, "ortho_per_chann[...]
        "39": {"class_type": "KSampler", "inputs": {"seed": 5544, "control_after_generate": "fixed", "steps": 24, "cfg": 5.5, "sampler_name": "dpmpp_2m", "scheduler": "karras", "denoise": 0.90, "model[...]
...
def run_suite_2_horology_steering():
    ...
    wf = {
        ...
        "2": {"class_type": "CLIPTextEncode", "inputs": {"text": "hero product macro shot of a sleek matte titanium luxury chronograph wristwatch on stone pedestal, neutral daylight studio lighting, c[...]
        ...
        "10": {"class_type": "KSampler", "inputs": {"seed": 8821, "control_after_generate": "fixed", "steps": 25, "cfg": 6.5, "sampler_name": "dpmpp_2m", "scheduler": "karras", "denoise": 1.0, "model"[...]
        "11": {"class_type": "VAEDecode", "inputs": {"samples": ["10", 0], "vae": ["1", 2]}},
        ...
        "14": {"class_type": "CLIPTextEncode", "inputs": {"text": "hero product macro shot of a sleek matte titanium luxury chronograph wristwatch on stone pedestal, deep underwater abyssal darkness, [...]
        "20": {"class_type": "GimbalCrossModalBridge", "inputs": {"llm_instruction": "deep abyssal underwater darkness, glowing cyan and electric sapphire bioluminescence, ethereal aquatic caustics", [...]
        "21": {"class_type": "GimbalCompass_Pro", "inputs": {"strength": 1.5, "mode": "Orthogonal_Projection", "clamp_output": True, "clamp_min": -8.0, "clamp_max": 8.0, "allow_batch_expand": False, "[...]
        ...
        "25": {"class_type": "KSampler", "inputs": {"seed": 8821, "control_after_generate": "fixed", "steps": 24, "cfg": 3.8, "sampler_name": "dpmpp_2m", "scheduler": "karras", "denoise": 0.60, "model[...]
        ...
        "30": {"class_type": "CLIPTextEncode", "inputs": {"text": "hero product macro shot of a sleek matte titanium luxury chronograph wristwatch on stone pedestal, darkroom forge, intense glowing mo[...]
        "31": {"class_type": "GimbalCrossModalBridge", "inputs": {"llm_instruction": "darkroom forge, glowing molten amber magma reflections, intense orange rim lighting, floating embers, pitch black [...]
        "32": {"class_type": "GimbalCompass_Pro", "inputs": {"strength": 1.5, "mode": "Orthogonal_Projection", "clamp_output": True, "clamp_min": -8.0, "clamp_max": 8.0, "allow_batch_expand": False, "[...]
        ...
        "35": {"class_type": "KSampler", "inputs": {"seed": 8821, "control_after_generate": "fixed", "steps": 24, "cfg": 3.8, "sampler_name": "dpmpp_2m", "scheduler": "karras", "denoise": 0.60, "model[...]
        "36": {"class_type": "VAEDecode", "inputs": {"samples": ["35", 0], "vae": ["1", 2]}},
...
def run_suite_3_parametric_desert_orbit():
    ...
    wf = {
        "1": {"class_type": "CheckpointLoaderSimple", "inputs": {"ckpt_name": CKPT}},
        "2": {"class_type": "CLIPTextEncode", "inputs": {"text": "futuristic parametric sculptural glass and sandstone desert pavilion emerging from rolling sand dunes, dramatic curvilinear architectu[...]
        "3": {"class_type": "CLIPTextEncode", "inputs": {"text": "blurry, low quality, cartoon, watermark, noisy, deformed", "clip": ["1", 1]}},
        "4": {"class_type": "EmptyLatentImage", "inputs": {"width": 1024, "height": 1024, "batch_size": 1}},
        ...
        "30": {"class_type": "KSampler", "inputs": {"seed": 9191, "control_after_generate": "fixed", "steps": 24, "cfg": 6.0, "sampler_name": "dpmpp_2m", "scheduler": "karras", "denoise": 1.0, "model"[...]
...
def run_suite_4_armchair_material_matrix():
    ...
    wf = {
        ...
        "2": {"class_type": "CLIPTextEncode", "inputs": {"text": "minimalist architectural sculptural curved lounge armchair in raw natural oat linen and blonde ash wood frame, concrete architectural [...]
        "3": {"class_type": "CLIPTextEncode", "inputs": {"text": "blurry, low quality, cartoon, distorted geometry, watermark, noisy", "clip": ["1", 1]}},
        "4": {"class_type": "EmptyLatentImage", "inputs": {"width": 1024, "height": 1024, "batch_size": 1}},
        
        "10": {"class_type": "KSampler", "inputs": {"seed": 9201, "control_after_generate": "fixed", "steps": 25, "cfg": 6.5, "sampler_name": "dpmpp_2m", "scheduler": "karras", "denoise": 1.0, "model"[...]
        "11": {"class_type": "VAEDecode", "inputs": {"samples": ["10", 0], "vae": ["1", 2]}},
        ...
        "21": {"class_type": "CLIPTextEncode", "inputs": {"text": "minimalist architectural sculptural curved lounge armchair made entirely of mirror polished liquid chrome and dark smoked glass, conc[...]
        "22": {"class_type": "GimbalCrossModalBridge", "inputs": {"llm_instruction": "mirror polished liquid chrome, dark smoked glass, mirror reflections, glossy metallic surface", "mapping_mode": "K[...]
        "23": {"class_type": "GimbalCompass_Pro", "inputs": {"strength": 0.85, "mode": "Orthogonal_Projection", "clamp_output": True, "clamp_min": -8.0, "clamp_max": 8.0, "allow_batch_expand": False, [...]
        ...
        "26": {"class_type": "KSampler", "inputs": {"seed": 9201, "control_after_generate": "fixed", "steps": 20, "cfg": 3.8, "sampler_name": "dpmpp_2m", "scheduler": "karras", "denoise": 0.38, "model[...]
        ...
        "31": {"class_type": "CLIPTextEncode", "inputs": {"text": "minimalist architectural sculptural curved lounge armchair in rich deep oxblood red velvet and brushed rose gold metal frame, concret[...]
        "32": {"class_type": "GimbalCrossModalBridge", "inputs": {"llm_instruction": "rich deep oxblood red velvet, brushed rose gold metal trim, warm velvet sheen, luxury bespoke upholstery", "mappin[...]
        "33": {"class_type": "GimbalCompass_Pro", "inputs": {"strength": 0.85, "mode": "Orthogonal_Projection", "clamp_output": True, "clamp_min": -8.0, "clamp_max": 8.0, "allow_batch_expand": False, [...]
        ...
        "36": {"class_type": "KSampler", "inputs": {"seed": 9201, "control_after_generate": "fixed", "steps": 20, "cfg": 3.8, "sampler_name": "dpmpp_2m", "scheduler": "karras", "denoise": 0.38, "model[...]
